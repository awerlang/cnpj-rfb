...
function fetchReceitaWs(cnpj) {
    return new Promise((resolve, reject) => {
        request({ url: urlReceitaWs + cnpj, timeout: 5000 }, (err, wsResponse) => {
...
function fetchRfb(cnpj, authCookie) {
    return new Promise((resolve, reject) => {
        ...
        request({ url: urlCnpj, jar: jar, qs: { cnpj: cnpj }, encoding: null }, (err, cnpjResponse) => {
            if (err) return reject(err)
            ...
            request({ url: urlQsa, jar: jar, encoding: null }, (err, qsaResponse) => {
                if (err) return reject(err)
